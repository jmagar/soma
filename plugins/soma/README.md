...
Multi-platform plugin package that connects Claude Code, Codex, and Gemini CLI to the Soma MCP server.

The default MCP connection is the installed local stdio adapter:
`soma mcp`. For platform deployments, set
`soma_api_url` to the deployed `soma-server` REST API base URL so the local
...
`soma-server` HTTP profile can opt into this monitor from the Claude manifest.
