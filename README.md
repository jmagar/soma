...
`rmcp-template` is a batteries-included RMCP server runtime and shipping binary
...
template. The default product path is to run `rtemplate` or `rtemplate-server`,
...
```bash
cargo run --bin rtemplate -- hello-local --name Alice
```
