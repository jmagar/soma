...
│   └── api.ts              # Typed REST client for Soma server
...
- **API calls go through `lib/api.ts`** — do not fetch the Soma server directly from page components; add methods to `api.ts` and call those.
...
3. Keep service/action metadata in `lib/soma.ts`
