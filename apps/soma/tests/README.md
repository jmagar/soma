...
bash apps/soma/tests/mcporter/test-mcp.sh --parallel

# Verbose raw output
bash apps/soma/tests/mcporter/test-mcp.sh --verbose

# Default target is http://localhost:40060/mcp (the `just dev` port).
# Override target when testing another deployment:
SOMA_MCP_HOST=127.0.0.1 SOMA_MCP_PORT=3100 bash apps/soma/tests/mcporter/test-mcp.sh
```
...
`apps/soma/src/lib.rs` exports helpers under `soma::testing`:
