...
check "Claude plugin name is rtemplate" "test \"\$(jq -er '.name' '${CLAUDE_PLUGIN_JSON}')\" = 'rtemplate'"
...
check "Gemini MCP server is named rtemplate" "jq -er '.mcpServers.rtemplate' '${GEMINI_EXTENSION_JSON}'"
check "Gemini MCP transport is stdio" "jq -er '.mcpServers.rtemplate.type == \"stdio\"' '${GEMINI_EXTENSION_JSON}'"
check "Gemini MCP command uses PATH binary" "jq -er '.mcpServers.rtemplate.command == \"rtemplate\"' '${GEMINI_EXTENSION_JSON}'"
check "Gemini MCP args run stdio mode" "jq -er '.mcpServers.rtemplate.args == [\"mcp\"]' '${GEMINI_EXTENSION_JSON}'"
check "Gemini MCP env maps API URL" "jq -er '.mcpServers.rtemplate.env.RTEMPLATE_API_URL == \"\${settings.rtemplate_api_url}\"' '${GEMINI_EXTENSION_JSON}'"
check "Gemini MCP env maps API key" "jq -er '.mcpServers.rtemplate.env.RTEMPLATE_API_KEY == \"\${settings.rtemplate_api_key}\"' '${GEMINI_EXTENSION_JSON}'"

check "MCP config exists" "test -f '${MCP_JSON}'"
check "MCP config is valid JSON" "jq empty '${MCP_JSON}'"
check "MCP server is named rtemplate" "jq -er '.mcpServers.rtemplate' '${MCP_JSON}'"
check "MCP transport is stdio" "jq -er '.mcpServers.rtemplate.type == \"stdio\"' '${MCP_JSON}'"
check "MCP command uses PATH binary" "jq -er '.mcpServers.rtemplate.command == \"rtemplate\"' '${MCP_JSON}'"
check "MCP args run stdio mode" "jq -er '.mcpServers.rtemplate.args == [\"mcp\"]' '${MCP_JSON}'"
check "MCP env maps API URL" "jq -er '.mcpServers.rtemplate.env.RTEMPLATE_API_URL == \"\${user_config.rtemplate_api_url}\"' '${MCP_JSON}'"
check "MCP env maps API key" "jq -er '.mcpServers.rtemplate.env.RTEMPLATE_API_KEY == \"\${user_config.rtemplate_api_key}\"' '${MCP_JSON}'"

check "hooks config exists" "test -f '${HOOKS_JSON}'"
check "hooks config is valid JSON" "jq empty '${HOOKS_JSON}'"
check "SessionStart runs plugin setup" "jq -er '.hooks.SessionStart[]?.hooks[]?.command == \"rtemplate setup plugin-hook\"' '${HOOKS_JSON}'"
check "ConfigChange runs plugin setup" "jq -er '.hooks.ConfigChange[]? | select(.matcher == \"user_settings\") | .hooks[]?.command == \"rtemplate setup plugin-hook\"' '${HOOKS_JSON}'"
