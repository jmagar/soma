...
- **CI runs on self-hosted runners** — Linux jobs on dookie (`[self-hosted, Linux, rmcp-template, dookie]`), Windows on steamy. `ci.yml` triggers on `push` to main and `pull_request` (PR-based workflow). This is a **private** repo, so only collaborators + dependabot can trigger CI — no untrusted fork PRs are possible. Full setup, the strict `HOME`/`CARGO_HOME` isolation, and the troubleshooting log are in [`docs/LINUX-RUNNER.md`](docs/LINUX-RUNNER.md) (+ `docs/WINDOWS-RUNNER.md`).
