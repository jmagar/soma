#!/usr/bin/env bash
# =============================================================================
# test-mcp.sh — Integration smoke-test for the Soma MCP server
#
# CUSTOMIZE: Replace all occurrences of "soma"/"Example"/"EXAMPLE" with your
...
#   transport harness.
#
#   CUSTOMIZE: Adapt these checks to your service's actual response shapes.
...
#   SOMA_MCP_HOST  (default: localhost)
#   SOMA_MCP_PORT  (default: 40060)
#   SOMA_MCP_TOKEN (optional; omit for no-auth dev mode)
#
# Usage:
#   ./crates/soma/tests/mcporter/test-mcp.sh [--timeout-ms N] [--parallel] [--verbose]
#
# Options:
...
#
# CUSTOMIZE: To add more actions, copy the pattern from suite_core below:
...
load_env() {
  ...
  # CUSTOMIZE: Replace SOMA_MCP_HOST/PORT with your service's env var names.
  local host="${SOMA_MCP_HOST:-localhost}"
  # Remap bind address 0.0.0.0 → localhost for outbound connections
  [[ "${host}" == "0.0.0.0" ]] && host="localhost"
  local port="${SOMA_MCP_PORT:-40060}"
  MCP_URL="http://${host}:${port}/mcp"

  # CUSTOMIZE: Replace SOMA_MCP_TOKEN with your service's token env var.
  local token="${SOMA_MCP_TOKEN:-}"
  MCPORTER_HEADER_ARGS=()
  if [[ -n "${token}" ]]; then
  ...
  if [[ ${#MCPORTER_HEADER_ARGS[@]} -gt 0 ]]; then
    ...
    log_info "Auth: none (SOMA_MCP_TOKEN unset — server must be in no-auth mode)"
  fi
}
...
smoke_test_server() {
  ...
  if [[ "${health_status}" != "ok" ]]; then
    ...
    # CUSTOMIZE: Replace "soma" with your service name in the diagnostic messages.
    log_error "Is the soma-mcp server running?  just dev   or   just docker-up"
    log_error "Then retry:  ./crates/soma/tests/mcporter/test-mcp.sh"
    return 2
  fi
...
# CUSTOMIZE: Replace "soma" tool name with your tool name.
...
#
# CUSTOMIZE: Use this for all checks where you can predict the response value.
...
# =============================================================================
# TEST SUITES
# CUSTOMIZE: Rename these suites and adapt the test cases for your service.
# =============================================================================
...
suite_auth() {
  ...
  # CUSTOMIZE: Replace SOMA_MCP_TOKEN with your token env var name.
  if [[ -z "${SOMA_MCP_TOKEN:-}" ]]; then
    skip_test "auth: unauthenticated /mcp returns 401" "SOMA_MCP_TOKEN unset"
    skip_test "auth: bad token returns 401"             "SOMA_MCP_TOKEN unset"
    return
  fi
...
# ── suite_core ────────────────────────────────────────────────────────────────
# CUSTOMIZE: This is the main test suite. Each test here is a semantic check —
...
suite_core() {
  printf '\n%b== soma tool — core actions ==%b\n' "${C_BOLD}" "${C_RESET}" | tee -a "${LOG_FILE}"

  # ── greet ───────────────────────────────────────────────────────────────────
  # CUSTOMIZE: Replace "soma" with your tool name, adapt the action and response.
...
# CUSTOMIZE: The schema resource is exposed by the soma as:
#           soma://schema/mcp-tool
#           Replace "soma" with your service name in the URI.
suite_schema_resource() {
  printf '\n%b== schema resource ==%b\n' "${C_BOLD}" "${C_RESET}" | tee -a "${LOG_FILE}"

  # Fetch the schema resource via mcporter when supported. The wrapper falls
  # back to raw JSON-RPC for older mcporter versions.
  local resource_uri="soma://schema/mcp-tool"
...
main() {
  ...
  printf '%b%s%b\n' "${C_BOLD}" "$(printf '=%.0s' {1..65})" "${C_RESET}"
  # CUSTOMIZE: Replace "soma-mcp" with your service name in this banner.
  printf '%b  soma-mcp integration smoke-test%b\n' "${C_BOLD}" "${C_RESET}"
  ...
  smoke_test_server || {
    ...
    log_error ""
    # CUSTOMIZE: Replace port 40060 and service name in these diagnostic messages.
    ...
    log_error "  docker ps | grep soma-mcp        # check Docker container"
    exit 2
  }
