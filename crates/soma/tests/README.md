...
Tests cover CLI parsing, service/action behavior, REST routes, plugin contracts, Soma invariants, and live MCP HTTP integration.
...
Tests MCP action behavior below HTTP. These use `soma::testing::loopback_state()` and the stub `SomaClient`, so no real credentials or upstream service are required.
...
> Soma rule: add one semantic test per business action. Assert response values, not only JSON validity.
...
### `soma_invariants.rs` — Portable automation contract

Rust tests that make Soma automation visible and hard to drift:
...
bash crates/soma/tests/mcporter/test-mcp.sh --parallel

# Verbose raw output
bash crates/soma/tests/mcporter/test-mcp.sh --verbose

# Default target is http://localhost:40060/mcp (the `just dev` port).
# Override target when testing another deployment:
SOMA_MCP_HOST=127.0.0.1 SOMA_MCP_PORT=3100 bash crates/soma/tests/mcporter/test-mcp.sh
```
...
|---|---|
| `suite_auth` | Missing and bad bearer tokens return `401` when `SOMA_MCP_TOKEN` is set. |
| `suite_core` | `greet`, `echo`, `status`, and `help` return semantically correct values. |
| `suite_schema_resource` | `soma://schema/mcp-tool` resolves and contains a valid tool schema with `inputSchema.properties.action`. |

The script logs all calls to `/tmp/test-mcp.<timestamp>.log`.

> Soma rule: adapt `suite_core` and resource assertions when adding service-specific actions or resources. Non-destructive live actions only.

---

## Test helpers

`crates/soma/src/lib.rs` exports helpers under `soma::testing`:
...
The stub client uses an empty `SOMA_API_URL`, so service-layer tests remain deterministic and do not contact a real upstream.
