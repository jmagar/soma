...
fn setup_check(config: &Config, no_repair: bool) -> SetupReport {
    ...
    if config.soma.api_url.is_empty() {
        report.blocking_failures.push(SetupFailure {
            code: "missing_soma_api_url",
            message: "SOMA_API_URL is required".into(),
        });
    }
    if config.soma.api_key.is_empty() {
        report.blocking_failures.push(SetupFailure {
            code: "missing_soma_api_key",
            message: "SOMA_API_KEY is required".into(),
        });
    }
...
fn check_auth(config: &Config, report: &mut SetupReport) {
    ...
    if config.mcp.auth.mode == AuthMode::OAuth {
        ...
        report.blocking_failures.push(SetupFailure {
            ...
            message: "SOMA_MCP_TOKEN is required unless no_auth or OAuth mode is enabled".into(),
        });
    }
}
...
fn write_env(data_dir: &Path, config: &Config) -> Result<()> {
    let mut lines = vec![
        dotenv_assignment("SOMA_API_URL", &config.soma.api_url)?,
        dotenv_assignment("SOMA_API_KEY", &config.soma.api_key)?,
    ...
    if let Some(token) = config.mcp.api_token.as_deref().filter(|v| !v.is_empty()) {
        lines.push(dotenv_assignment("SOMA_MCP_TOKEN", token)?);
    }
    if config.mcp.auth.mode == AuthMode::OAuth {
        lines.push("SOMA_MCP_AUTH_MODE=oauth".into());
        if let Some(v) = &config.mcp.auth.public_url {
            lines.push(dotenv_assignment("SOMA_MCP_PUBLIC_URL", v)?);
        }
