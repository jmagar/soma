...
fn with_plugin_data<T>(dir: &std::path::Path, f: impl FnOnce() -> T) -> T {
    ...
    let _restore = EnvRestore {
        old: std::env::var_os("SOMA_HOME"),
    };
    unsafe {
        std::env::set_var("SOMA_HOME", dir);
    }
    f()
}
...
fn dotenv_values_quote_special_characters_and_escape_quotes() {
    assert_eq!(
        super::dotenv_assignment("SOMA_API_KEY", "secret # \"quoted\"").unwrap(),
        "SOMA_API_KEY=\"secret # \\\"quoted\\\"\""
    );
}

#[test]
fn dotenv_values_reject_newlines() {
    let error = super::dotenv_assignment("SOMA_API_KEY", "line\nbreak").unwrap_err();
