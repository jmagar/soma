...
//! ```
//!
//! The service layer tests verify that SomaService correctly delegates to
//! SomaClient and that any transformations or caching are correct.
//!
//! **Customize**: These tests use real SomaClient instances in offline stub
...
async fn test_service_greet_delegates_to_client() {
    // CUSTOMIZE: Test that the service correctly passes parameters through to the client.
...
async fn test_service_greet_with_name_passes_name_through() {
    // CUSTOMIZE: Verify the service passes parameters through unchanged.
...
async fn test_service_echo_returns_exact_message() {
    // CUSTOMIZE: Test round-trip fidelity at the service layer.
    let service = stub_service();
...
async fn test_service_status_returns_ok() {
    // CUSTOMIZE: Status checks at the service layer should pass through correctly.
    let service = stub_service();
...
fn assert_scaffold_contract_shape(value: &Value) {
    assert_eq!(value["kind"], "soma_scaffold_intent");
