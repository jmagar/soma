...
fn push_configured_origin(origins: &mut Vec<String>, origin: &str) {
    let Some(origin) = extract_configured_origin_with_label(origin, "SOMA_MCP_ALLOWED_ORIGINS")
    else {
        return;
    };
    origins.push(origin);
...
fn push_public_url_hosts(hosts: &mut Vec<String>, url: &str, listen_port: u16) {
    let Ok(parsed) = url::Url::parse(url) else {
        tracing::warn!(public_url = url, "SOMA_MCP_PUBLIC_URL is not a valid URL");
        return;
    };
    ...
    if host.contains('*') {
        tracing::warn!(host, "SOMA_MCP_PUBLIC_URL host contains wildcard; skipping");
        return;
    }
...
fn extract_origin(url: &str) -> Option<String> {
    extract_origin_with_label(url, "SOMA_MCP_PUBLIC_URL")
}
