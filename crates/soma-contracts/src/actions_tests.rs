    ...
    for spec in ACTION_SPECS {
        if spec.transport.cli() {
            ...
            assert_eq!(
                ...
                "{} CLI command should match the action name unless Soma explicitly adds an alias map",
                spec.name
            );
            assert!(
...
fn mcp_args_parse_flat_shape() {
    let action = SomaAction::from_mcp_args(&json!({ "action": "echo", "message": "hello" }))
...
fn rest_args_parse_nested_params_shape() {
    let action = SomaAction::from_rest("greet", &json!({ "name": "Alice" }))
...
fn missing_action_is_validation_error() {
    let error = SomaAction::from_mcp_args(&json!({})).unwrap_err();
...
fn non_string_action_is_wrong_type_validation_error() {
    let error = SomaAction::from_mcp_args(&json!({ "action": 42 })).unwrap_err();
...
fn echo_rejects_missing_and_empty_message() {
    let missing = SomaAction::from_mcp_args(&json!({ "action": "echo" })).unwrap_err();
    ...
    let empty = SomaAction::from_rest("echo", &json!({ "message": "" })).unwrap_err();
    assert!(empty.to_string().contains("`message` is required"));
}

#[test]
fn string_params_reject_wrong_json_type() {
    let greet = SomaAction::from_rest("greet", &json!({ "name": 42 })).unwrap_err();
    assert!(greet.to_string().contains("`name` must be a string"));

    let echo = SomaAction::from_mcp_args(&json!({
        "action": "echo",
...
fn rest_rejects_mcp_only_actions() {
    let error = SomaAction::from_rest("scaffold_intent", &json!({})).unwrap_err();
    assert!(error.to_string().contains("not available over REST"));

    let error = SomaAction::from_rest("elicit_name", &json!({})).unwrap_err();
    assert!(error.to_string().contains("not available over REST"));
}

#[test]
fn rest_missing_action_preserves_missing_action_error() {
    let error = SomaAction::from_rest("", &json!({})).unwrap_err();
    assert_eq!(error.to_string(), "action is required");
}

#[test]
fn unknown_action_mentions_help() {
    let error = SomaAction::from_rest("missing", &json!({})).unwrap_err();
...
fn all_parse_errors_are_classified_as_validation_errors() {
    let cases: &[anyhow::Error] = &[
        SomaAction::from_mcp_args(&json!({})).unwrap_err(),
        SomaAction::from_mcp_args(&json!({ "action": "echo" })).unwrap_err(),
        SomaAction::from_rest("echo", &json!({ "message": "" })).unwrap_err(),
        SomaAction::from_rest("greet", &json!({ "name": 42 })).unwrap_err(),
        SomaAction::from_rest("scaffold_intent", &json!({})).unwrap_err(),
        SomaAction::from_rest("missing", &json!({})).unwrap_err(),
    ];
