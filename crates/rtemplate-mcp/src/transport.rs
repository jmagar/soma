...
use rtemplate_contracts::config::McpConfig;
...
pub fn streamable_http_service(
    state: rtemplate_runtime::server::AppState,
...
fn push_public_url_hosts(hosts: &mut Vec<String>, url: &str, listen_port: u16) {
    let Ok(parsed) = url::Url::parse(url) else {
        tracing::warn!(
            public_url = url,
            "RTEMPLATE_MCP_PUBLIC_URL is not a valid URL"
        );
        return;
    };
    ...
    if host.contains('*') {
        tracing::warn!(
            host,
            "RTEMPLATE_MCP_PUBLIC_URL host contains wildcard; skipping"
        );
        return;
    }
...
fn extract_origin(url: &str) -> Option<String> {
    extract_origin_with_label(url, "RTEMPLATE_MCP_PUBLIC_URL")
}
