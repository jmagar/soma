...
///
/// Soma looks for `<data_dir>/config.toml` (e.g. `~/.soma/config.toml`).
...
pub fn check_config_file(data_dir: &Path) -> DoctorCheck {
    ...
    if config_path.exists() {
        ...
        // CUSTOMIZE: Change `pass` → `fail` if config.toml is mandatory.
        DoctorCheck {
            category: "config",
...
pub fn check_required_var(var_name: &str, value: &str) -> DoctorCheck {
    if !value.is_empty() {
        ...
        DoctorCheck::fail(
            ...
            format!(
                "Not set.\n    \
                 → Add to ~/.soma/.env:  {var_name}=<your_value>\n    \
...
pub async fn check_upstream(base_url: &str) -> DoctorCheck {
    // CUSTOMIZE: Change "/health" to your upstream's actual probe path.
    let health_url = format!("{}/health", base_url.trim_end_matches('/'));

    // Use strict TLS by default. Set SOMA_DOCTOR_ACCEPT_INVALID_CERTS=true
    // only for dev environments with self-signed certificates.
    // CUSTOMIZE: Replace the env var prefix when adapting Soma.
    let accept_invalid_certs = std::env::var("SOMA_DOCTOR_ACCEPT_INVALID_CERTS")
                ...
                DoctorCheck::fail_with_latency(
                    ...
                    format!(
                        "HTTP {status} from {health_url}\n    \
                         ...
                         CUSTOMIZE: Verify the correct health endpoint path."
                    ),
                    elapsed,
                )
            }
            ...
            DoctorCheck::fail_with_latency(
                ...
                format!(
                    "Could not reach {health_url}: {e}\n    \
                     → Check SOMA_API_URL is correct and the service is running.\n    \
                     CUSTOMIZE: Replace SOMA_API_URL with your service's env var."
                ),
                elapsed,
            )
        }
        ...
        Err(e) => DoctorCheck::fail(
            ...
            format!(
                "Bind address {bind} is unavailable: {e}\n    \
                 → Set SOMA_MCP_PORT to a different port.\n    \
                 → Or stop the process using this address: ss -tlnp | grep :{port}\n    \
                 CUSTOMIZE: Replace SOMA_MCP_PORT with your service prefix."
            ),
        ),
    }
}
...
pub fn check_auth_config(config: &Config) -> DoctorCheck {
    match resolve_auth_policy_kind(config, config.mcp.trusted_gateway) {
        ...
        Ok(AuthPolicyKind::TrustedGatewayUnscoped) => DoctorCheck::pass(
            "auth",
            "Auth mode",
            "trusted gateway unscoped (SOMA_NOAUTH=true — upstream handles auth and authz)",
        ),
