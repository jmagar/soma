...
fn format_event(
    ...
    match (current, prev) {
        ...
        (ServerState::Up, _) => format!("[soma] UP — {} is healthy", base_url),
        // Went down
        (ServerState::Down, _) => format!(
            "[soma] DOWN — {} is unreachable (retrying every {}s)",
            base_url, interval_secs
        ),
        // Degraded (non-2xx)
        (ServerState::Degraded(code), _) => {
            format!("[soma] DEGRADED — {} returned HTTP {code}", base_url)
        }
    }
}
