...
// current_thread: this crate's own [dependencies] only enable tokio's `rt`
// feature (not `rt-multi-thread`), since a library shouldn't force a runtime
// flavor on its consumers - this example only needs a runtime to exist, so
// current_thread avoids adding an example-only dev-dependency for it.
