...
use rtemplate_contracts::config::{Config, ExampleConfig, McpConfig};
...
fn with_plugin_data<T>(dir: &std::path::Path, f: impl FnOnce() -> T) -> T {
    ...
    impl Drop for EnvRestore {
        fn drop(&mut self) {
            unsafe {
                match self.old.take() {
                    Some(value) => std::env::set_var("RTEMPLATE_HOME", value),
                    None => std::env::remove_var("RTEMPLATE_HOME"),
                }
            }
        }
    }
    ...
    let _restore = EnvRestore {
        old: std::env::var_os("RTEMPLATE_HOME"),
    };
    unsafe {
        std::env::set_var("RTEMPLATE_HOME", dir);
    }
    f()
}
