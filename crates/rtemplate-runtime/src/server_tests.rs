use super::*;
use rtemplate_contracts::config::{AuthConfig, ExampleConfig};
...
fn non_loopback_no_auth_without_gateway_is_rejected() {
    ...
    assert!(error.to_string().contains("RTEMPLATE_MCP_NO_AUTH=true"));
}
...
fn invalid_public_url_is_rejected() {
    ...
    assert!(error
        .to_string()
        .contains("RTEMPLATE_MCP_PUBLIC_URL is invalid"));
}
...
fn wildcard_public_url_is_rejected() {
    ...
    assert!(error
        .to_string()
        .contains("RTEMPLATE_MCP_PUBLIC_URL must not contain wildcard hosts"));
}
