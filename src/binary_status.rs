...
pub fn stale_binary_warning() -> Option<String> {
    if std::env::var_os("RTEMPLATE_SUPPRESS_STALE_BINARY_WARNING").is_some() {
        return None;
    }
...
pub fn warning_message(newer_input: &str) -> String {
    format!(
        "outdated rtemplate binary: {newer_input} is newer than the running executable. Rebuild with `cargo build --release --bin rtemplate` and install with `just install-local`."
    )
}
