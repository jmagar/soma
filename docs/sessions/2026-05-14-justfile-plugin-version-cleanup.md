...
transcript: /home/jmagar/.claude/projects/-home-jmagar-workspace-soma/e8aeb70d-b7af-4ff5-8534-1cfcaafe6850.jsonl
...
Audited Justfiles across all 9 repos in the rmcp-server family (soma, lab, axon_rust, syslog-mcp, rustifi, rustify, rustscale, unrust, apprise-mcp). Added 14 new recipes to soma's Justfile. Removed `"version"` from plugin manifests in 7 repos (lab and axon_rust were already clean). Fixed a pre-existing clippy lint in syslog-mcp as part of the same commit. Confirmed CLAUDE.md module map was already correct from the previous session.

## Sequence of Events

1. Verified CLAUDE.md module map — `src/server.rs` already listed correctly, no update needed
2. Read all 7 Justfiles in parallel (soma + lab + syslog-mcp + axon_rust + rustifi + rustify + rustscale + unrust + apprise-mcp)
3. Identified 14 recipes missing from soma present in ≥3 family repos
4. Rewrote soma Justfile with all additions + `build-web`, `web-watch`, `build-full`
...
- All 7 MCP server repos (soma, rustifi, rustify, rustscale, unrust, apprise-mcp, syslog-mcp) had `"version": "0.1.0"` or `"version": "0.20.0"` in their manifests
...
- soma Justfile was missing 14 recipes present in ≥3 family repos; notably `clean`, `deny`, `verify`, `fix`, `generate-cli`, `build-plugin`, `doctor`, `default`
- The `publish` recipe in soma did not update plugin.json files (unlike family members); fixed to match, then immediately removed the loop since manifests no longer carry versions
...
|------|------|--------|---------|
| `Justfile` | soma | Rewritten | Added 14 recipes; fixed `publish`; removed plugin version loop |
| `plugins/soma/.claude-plugin/plugin.json` | soma | Modified | Removed `"version"` field |
| `plugins/soma/.codex-plugin/plugin.json` | soma | Modified | Removed `"version"` field |
| `docs/PATTERNS.md` | soma | Modified | §13 + §36: added no-version convention with rationale |
...
# Push all 7 repos
for repo in soma rustifi rustify rustscale unrust apprise-mcp syslog-mcp; do
...
- **Propagate new Justfile recipes to all family repos**: Only the plugin version change was propagated. The 14 new recipes added to soma were not backported to rustifi/rustify/etc. — those repos have their own cadence and Soma is the source of truth.
