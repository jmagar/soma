...
transcript: /home/jmagar/.claude/projects/-home-jmagar-workspace-soma/eba6937e-a046-4f2c-ac81-63acfb793ec7.jsonl
...
8. Created canonical `docs/RUST.md` in `soma` covering system prerequisites, full global config, rationale for every setting, and documented per-repo overrides.
9. Created focused `docs/RUST.md` in all 8 derived repos.
10. Updated `docs/DOCS.md` (added RUST.md to directory tree) and `docs/QUICKSTART.md` (added clang/mold to prerequisites) in `soma`.
...
- **9 family repos confirmed**: `soma`, `lab`, `axon_rust`, `syslog-mcp`, `rustifi`, `rustify`, `apprise-mcp`, `rustscale`, `unrust` — listed explicitly in `soma/docs/CLAUDE.md`.
...
- **Canonical RUST.md in soma, focused stubs in derived repos**: derived repos point to the canonical reference rather than duplicating content, keeping them maintainable.
...
- **Standard alias format `"run --package xtask --"`** (not `-p` shorthand): matches Soma's convention; axon_rust was using `-p` which was also changed.
...
for repo in soma apprise-mcp syslog-mcp rustifi rustscale rustify unrust axon_rust lab; do
...
| Build-setup documentation | No `docs/RUST.md` anywhere in family | Canonical file in `soma`; focused stubs in all 8 derived repos |
...
### Follow-on tasks
- **Apply the same `.gitignore` fix to Soma's `.gitignore`**: `soma` itself uses the same boilerplate `.gitignore` that had the `config.toml` bug. Verify whether `soma`'s own `.gitignore` has the bare form and fix if so.
- **Propagate `docs/RUST.md` to any new repos** derived from Soma going forward — the `docs/CLAUDE.md` Template Adaptation checklist should mention it.
...
- **`rust-version` alignment**: derived repos use `1.86`, `soma` uses `1.90`. Decide whether to align upward across the family.
