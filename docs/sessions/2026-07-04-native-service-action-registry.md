...
`the retired REST action-envelope route` envelope is covered by an integration test.

## Review Follow-Ups

- Kept REST params strict for direct routes and added coverage for unknown
  fields.
- Moved action metadata generation to `soma-service` and updated docs,
...
- Bumped the shipped Soma component to `0.4.3` and updated `anyhow` to
