...
Refactor the soma Rust MCP server: create a CLAUDE.md for `apps/web/` guiding Aurora design system usage, then fix improper module organization (`src/mcp/` containing REST API code), enforce the no-`mod.rs` rule via a git hook, update `docs/PATTERNS.md` to match, add `deny.toml`, and push everything.
...
| Public API | `soma::mcp::{AppState, AuthPolicy, router}` | `soma::server::{AppState, AuthPolicy, router}` |
...
- **Breaking public API**: `soma::mcp::{AppState, AuthPolicy}` is now `soma::server::*`. Any downstream crate importing from the library will break. Rollback: revert `src/lib.rs` module declarations and restore old `src/mcp.rs`.
