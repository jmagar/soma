...
pr: "#85 Codex/conformance harness fixtures https://github.com/jmagar/soma/pull/85"
beads: soma-uw6h, soma-11ju, soma-q25n
---
...
The session centered on turning Soma into a reusable provider-based runtime, capturing that direction in specs and GitHub issue #83, then fixing the failing CI checks on PR #85. The final request was to save the session as markdown with repository maintenance context.
...
1. Removed the public REST action-envelope route from Soma.
...
- `the retired REST action-envelope route` was removed earlier in the session; the repo now points users toward direct REST routes rather than a REST action envelope.
...
- PR #85 initially failed `Template Contracts` because `crates/soma-mcp/src/conformance.rs` lacked the required sibling test file; after the fix, the latest `Template Contracts` failure moved to stale generated docs: `docs/generated/scripts-index.md is stale; run cargo xtask generate-docs`.
...
- Conformance fixture tests were moved from an inline test module to `crates/soma-mcp/src/conformance_tests.rs` because this repo enforces sibling `*_tests.rs` files.
...
| modified | `README.md` | - | Updated REST/API guidance after removing `retired REST action-envelope route`. | Commit `fb4fbcf` |
...
| modified | `apps/web/lib/api.ts` | - | Removed generated API use of `retired REST action-envelope route`. | Commit `fb4fbcf` |
| modified | `apps/web/lib/soma.ts` | - | Updated template-generated web references. | Commit `fb4fbcf` |
| modified | `crates/soma/src/routes.rs` | - | Removed route wiring for `the retired REST action-envelope route`. | Commit `fb4fbcf` |
| modified | `crates/soma/tests/api_routes.rs` | - | Updated API route tests for direct REST behavior. | Commit `fb4fbcf` |
| modified | `crates/soma-api/src/api.rs` | - | Removed the REST action-envelope handler. | Commit `fb4fbcf` |
| modified | `crates/soma-api/src/api_tests.rs` | - | Updated API tests after handler removal. | Commit `fb4fbcf` |
| modified | `crates/soma-contracts/src/actions.rs` | - | Kept action metadata/routes consistent with direct REST surface. | Commit `fb4fbcf` |
| modified | `crates/soma-web/assets/source/app/api/page.tsx` | - | Updated embedded web scaffold source. | Commit `fb4fbcf` |
| modified | `crates/soma-web/assets/source/app/tools/page.tsx` | - | Updated embedded web scaffold source. | Commit `fb4fbcf` |
| modified | `crates/soma-web/assets/source/lib/api.test.ts` | - | Updated embedded web scaffold source tests. | Commit `fb4fbcf` |
| modified | `crates/soma-web/assets/source/lib/api.ts` | - | Updated embedded web scaffold API client. | Commit `fb4fbcf` |
| modified | `crates/soma-web/assets/source/lib/soma.ts` | - | Updated embedded web scaffold template strings. | Commit `fb4fbcf` |
| modified | `docs/API.md` | - | Removed stale `retired REST action-envelope route` documentation. | Commit `fb4fbcf` |
...
| modified | `docs/generated/openapi.json` | - | Removed `retired REST action-envelope route` from generated OpenAPI. | Commit `fb4fbcf` |
...
| modified | `crates/soma-mcp/src/conformance.rs` | - | Replaced inline tests with a sibling test module reference. | Commit `b86584a` |
| created | `crates/soma-mcp/src/conformance_tests.rs` | - | Sibling tests for conformance fixtures. | Commit `b86584a` |
...
|---|---|---|---|---|
| `soma-uw6h` | Not fully inspected in this save pass | Closed earlier in the session | closed | Observed in `.beads/interactions.jsonl`: route removal work was closed with reason `Removed the retired REST action-envelope route route, handler, OpenAPI schema support, web references, and documented direct REST-only guidance.` |
| `soma-11ju` | Specify dynamic provider runtime contract | Created and closed | closed | Tracked creation of the provider runtime spec, manifest contract, and GitHub issue #83 update. |
| `soma-q25n` | Resolve PR 85 generated-docs failure and main conflict | Created during save-session maintenance | open | Tracks the observed remaining PR #85 work: conflict with `main` and stale generated docs in `Template Contracts`. |
...
`bd show soma-11ju` confirmed the provider-runtime contract task was closed. `bd create` created `soma-q25n` for the remaining PR #85 conflict and generated-docs failure. No other beads were modified during this save pass.
...
The latest `Template Contracts` log reported `docs/generated/scripts-index.md is stale; run cargo xtask generate-docs`. This was not fixed during the save-session command because the save contract commits only the generated session artifact; follow-up is tracked in `soma-q25n`.
...
- **Rust tooling.** Used `cargo update`, `cargo fmt`, `cargo xtask check-test-siblings`, `cargo deny check all`, `cargo test -p soma-mcp`, and `just conformance active 41060`.
...
| `gh api /repos/jmagar/soma/actions/jobs/85186138767/logs` | Showed `anyhow v1.0.102` was denied and needed `>=1.0.103`. |
| `gh api /repos/jmagar/soma/actions/jobs/85186126509/logs` | Showed the conformance workflow failed with `just: command not found`. |
| `gh api /repos/jmagar/soma/actions/jobs/85186138781/logs` | Showed `crates/soma-mcp/src/conformance.rs` needed `conformance_tests.rs`. |
...
| `cargo test -p soma-mcp` | Passed 44 tests. |
...
| `bd create --title "Resolve PR 85 generated-docs failure and main conflict" ...` | Created `soma-q25n`. |
...
- A later `Template Contracts` run failed because `docs/generated/scripts-index.md` is stale; unresolved and tracked by `soma-q25n`.
- `gh pr view 85` reported PR #85 as `CONFLICTING`; unresolved and tracked by `soma-q25n`.
...
| area | before | after |
|---|---|---|
| REST API | Public `retired REST action-envelope route` action envelope existed. | Direct REST routes are the intended public API shape; `retired REST action-envelope route` was removed. |
...
| `cargo test -p soma-mcp` | MCP crate tests pass. | 44 passed. | pass |
...
- PR #85: https://github.com/jmagar/soma/pull/85
- Issue #83: https://github.com/jmagar/soma/issues/83
...
1. Resolve `soma-q25n`: update PR #85 from `main`, resolve conflicts, and run `cargo xtask generate-docs`.
