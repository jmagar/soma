...
beads: rmcp-template-821, rmcp-template-sk6, rmcp-template-chl, rmcp-template-hi8, rmcp-template-lo4
---
...
The session centered on continuing the `rmcp-template` extraction and hardening work: port shared action/error patterns into the template, keep REST/CLI/MCP parity registry-driven, and then address the order-sensitive `rtemplate-auth` Google token test exposed by full parallel `cargo test`.
...
2. Added shared error types in `crates/rtemplate-contracts/src/errors.rs` and wired service-level classification through `rtemplate-service`.
...
5. Observed full parallel `cargo test` exposing a Google OAuth test flake, created `rmcp-template-sk6`, found the dropped `MockServer` lifetime bug, and fixed it.
...
- The shared error vocabulary now lives in `crates/rtemplate-contracts/src/errors.rs:6` and maps service error kind to HTTP status, retryability, REST payloads, and MCP payloads.
- REST action execution now flows through `run_rest_action_request` and `rest_error_response` in `crates/rtemplate-api/src/api.rs:229`, so parse, scope, execution, and error mapping share one wrapper.
- CLI action metadata is part of `ActionSpec` at `crates/rtemplate-contracts/src/actions.rs:153`, allowing parser parity tests to check registry coverage instead of hand-maintained expectations.
- REST route parity is asserted against `ACTION_SPECS` in `crates/rmcp-template/tests/api_routes.rs:61`.
- The auth flake was caused by a test helper returning a provider pointed at a `wiremock::MockServer` that had already been dropped; the fixed wrapper keeps the server alive in `crates/rtemplate-auth/src/google.rs:776`.

## Technical Decisions

- Kept the canonical error taxonomy in `rtemplate-contracts`, with service-specific classification in `rtemplate-service`, so REST/MCP/CLI can depend on one `ToolError` shape without creating dependency cycles.
...
| modified | `crates/rtemplate-contracts/src/actions.rs` | - | Add CLI metadata to `ActionSpec` and preserve REST route metadata. | `crates/rtemplate-contracts/src/actions.rs:153` |
| created | `crates/rtemplate-contracts/src/errors.rs` | - | Define shared `ServiceErrorKind`, `ToolError`, REST payload, MCP payload, and execution classification. | `crates/rtemplate-contracts/src/errors.rs:6` |
| created | `crates/rtemplate-contracts/src/errors_tests.rs` | - | Add generated-docs/cargo-generate PR tests for error contracts after fast-forward. | `e4a290b` |
| modified | `crates/rtemplate-contracts/src/lib.rs` | - | Export the new `errors` module. | `185b0fd` |
| modified | `crates/rtemplate-service/src/lib.rs` | - | Add `classify_service_error` and route scaffold/action validation into the shared taxonomy. | `185b0fd` |
| modified | `crates/rtemplate-api/src/api.rs` | - | Add shared REST action wrapper and structured error response mapping. | `crates/rtemplate-api/src/api.rs:229` |
| modified | `crates/rtemplate-mcp/src/rmcp_server.rs` | - | Replace MCP-local execution/validation error payload builders with shared `ToolError` payloads. | `185b0fd` |
| modified | `crates/rtemplate-mcp/src/rmcp_server_tests.rs` | - | Update MCP structured-error tests to use shared classification. | `185b0fd` |
| modified | `crates/rtemplate-cli/src/lib.rs` | - | Print CLI errors from shared `ToolError` fields. | `crates/rtemplate-cli/src/lib.rs:196` |
| modified | `crates/rtemplate-cli/src/cli_tests.rs` | - | Add registry-driven CLI parser coverage and CLI error-format checks. | `185b0fd` |
| modified | `crates/rmcp-template/tests/api_routes.rs` | - | Add REST route/action registry parity test. | `crates/rmcp-template/tests/api_routes.rs:61` |
| modified | `crates/rtemplate-auth/src/google.rs` | - | Keep WireMock server alive for mocked Google providers. | `crates/rtemplate-auth/src/google.rs:776` |
...
| modified | `README.md`, `docs/CARGO_GENERATE.md`, `xtask/README.md`, `cargo-generate.toml`, `template/cargo-generate/post.rhai`, `xtask/src/*`, `config/Dockerfile` | - | Cargo-generate xtask PR moved rewrite logic into Rust and updated docs/runtime contracts. | `e4a290b` |

## Beads Activity

| bead | title | actions | final status | why it mattered |
|---|---|---|---|---|
| `rmcp-template-821` | Add shared error taxonomy and action parity metadata | Created, claimed, closed | closed | Tracked shared `ToolError`/`ServiceError`, REST wrapper, CLI metadata, parity tests, and inventory ignore work. |
| `rmcp-template-sk6` | Fix parallel rtemplate-auth Google token test flake | Created, claimed, closed | closed | Tracked the order-sensitive Google token test fix and required default-threaded `cargo test` proof. |
| `rmcp-template-chl` | Generate volatile docs and metadata from canonical specs | Read during maintenance | closed | Recent session context for generated docs, OpenAPI, plugin settings, and script docs automation. |
| `rmcp-template-hi8` | Generate remaining plugin and action documentation surfaces | Read during maintenance | closed | Recent session context for plugin manifest generation, REST route metadata in `ACTION_SPECS`, and stale-claim checking. |
| `rmcp-template-lo4` | Merge generated-docs automation into main | Read during maintenance | closed | Explained the `185b0fd` merge that included generated-docs automation and the shared-error work. |
...
`bd show` confirmed `rmcp-template-821` and `rmcp-template-sk6` are closed with reasons matching the observed implementation and verification. `bd dolt push` was run after both implementation commits before this save-session pass.
...
| command | result |
|---|---|
| `bd create ... rmcp-template-821` / `bd update rmcp-template-821 --claim` | Created and claimed the shared taxonomy task. |
| `cargo test -p rtemplate-contracts -p rtemplate-service -p rtemplate-api -p rtemplate-cli -p rtemplate-mcp` | Passed focused package tests after implementation. |
| `cargo test -p rmcp-template --test api_routes --test template_invariants` | Initially caught the optional `greet` body regression; passed after `optional_name_params`. |
| `cargo clippy -- -D warnings` | Passed after the shared taxonomy work and after the auth flake fix. |
| `cargo test` | Initially exposed a parallel `rtemplate-auth` Google token flake; passed after the mock lifetime fix. |
| `cargo test -p rtemplate-auth --lib` | Passed under default parallelism after the mock lifetime fix. |
| `for i in 1 2 3 4 5; do cargo test -p rtemplate-auth google::tests::google_exchange_rejects_ -- --nocapture || exit 1; done` | Passed five repeated runs of the formerly flaky Google negative tests. |
| `git fetch --prune` / `git pull --ff-only` | Confirmed local `main` was an ancestor of `origin/main` and fast-forwarded to `e4a290b`. |
| `bd close rmcp-template-821` and `bd close rmcp-template-sk6` | Closed both beads with implementation and verification reasons. |
...
|---|---|---|---|
| `cargo test -p rtemplate-contracts -p rtemplate-service -p rtemplate-api -p rtemplate-cli -p rtemplate-mcp` | Focused shared-surface tests pass. | Passed. | pass |
| `cargo test -p rmcp-template --test api_routes --test template_invariants` | REST registry parity and invariants pass. | Passed after optional `greet` fix. | pass |
| `cargo test -p rtemplate-auth --lib` | Auth tests pass with default test threading. | Passed, 133 tests. | pass |
| `for i in 1 2 3 4 5; do cargo test -p rtemplate-auth google::tests::google_exchange_rejects_ -- --nocapture || exit 1; done` | Formerly flaky Google negative tests pass repeatedly. | Passed 5 runs. | pass |
...
- Transcript path observed by the skill: `/home/jmagar/.claude/projects/-home-jmagar-workspace-rmcp-template/8dd2c014-bb7a-46f4-941d-3d4510a9f94d.jsonl` with 169 lines. It appears to be a Claude-session transcript from 2026-05-28, not a full Codex transcript for the current work.
