...
Test 6 custom MCP servers (unrust, rustscale, rustifi, rustify, apprise-mcp, soma) using mcporter with their external URLs, generate secure bearer tokens for all of them, and add them to the lab gateway at lab.tootie.tv with protected paths named after each repo.
...
| `/home/jmagar/workspace/soma/.env` | Replaced no-auth flags with `SOMA_MCP_TOKEN=<token>`; removed duplicate `SOMA_NOAUTH=true` |
...
mcporter list soma  # → 1 tool: example (5 actions)
...
| `mcporter list soma` | Tool schema for example | 1 tool · 41ms | ✅ |
...
**Follow-on tasks:**
- Configure Authelia access rules for `https://mcp.tootie.tv/unrust`, `/rustify`, `/rustifi`, `/rustscale`, `/apprise-mcp`, `/soma` if required for gateway probing
