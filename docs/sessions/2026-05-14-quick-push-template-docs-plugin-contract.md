...
Run `quick-push` after the soma automation and documentation porting work.
...
```text
e8fa418 fix: sync Soma docs and plugin setup contract
```
...
| Area | Change |
|------|--------|
| Template port contract | Updated docs and comments from older `3000`/`3100` examples to the current `40060` template port where the reference was specific to soma. |
...
| AI instruction mirrors | Added `AGENTS.md` and `GEMINI.md` symlinks next to nested `CLAUDE.md` files in `apps/web`, `docs`, and `plugins/soma`. |
| Plugin distribution layout | Added `plugins/soma/bin/.gitkeep` so the plugin binary directory exists in git. |
...
- `just soma-check` passed after updating the plugin validator's expected `server_url`.
...
```bash
git add .
git commit -m "fix: sync Soma docs and plugin setup contract"
...
- Should `scripts/check-plugin-hook-contract.py` become part of Soma's default CI or stay as an operator-run cross-repo audit tool?
- Should the new plugin hook checker be added to `scripts/README.md`, `README.md`, `Justfile`, and `just soma-check` if it remains in this repo?
- Should the checker execute binaries by default only for soma, or keep `--execute` opt-in for all family repos because it may compile and run several workspaces?
