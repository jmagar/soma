...
This template was extracted from the current top-level READMEs for:
`unifi-rmcp`, `tailscale-rmcp`, `unraid-rmcp`, `apprise-rmcp`,
`gotify-rmcp`, `arcane-rmcp`, `yarr-rmcp`, `ytdl-mcp`, `synapse`,
`cortex`, `axon`, `lab`, `agentcast`, and `template-rmcp`.
...
| `template-rmcp` | Include scaffold profiles, generated action table, auth policies, adaptation checklist, docs map, and verification gates. |
...
Prefer `.env.example` or `config.example.toml` for longer setups:
